...
template <class T>
bool cmp(const std::pair<T, T>& lhs, const std::pair<T, T> rhs) {
    ...
    std::pair<T, T> ol = get(p0, lhs);
    std::pair<T, T> or = get(p0, rhs);
    return cross(ol, or) < 0;
} // конец вспомогательных фукнций
